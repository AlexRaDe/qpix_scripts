...
# Authors: PURM Students Summer 2024, NCD, ACG, SRAK
# Usage: python3 run_sampling_sweep.py
...
### USER-DEFINED VALUES ###
win_width_values = [500e-6, 1e-3,100e-3, 1, 2, 4, 8, 10, 20, 40]  # List of window widths to test
#win_width_values = [20, 40]
win_wait = 5e-6
reset_width = 5e-6
rst_cal_gap = 100e-9
trials_num = 10
interface = 2
external_clock = True
output_file = 'outputs/sampling_resultsb4-717_chan15.txt'
...
for win_width in win_width_values:
    print(f'Sampling working channels with win width: {win_width}, win_wait: {win_wait}, reset_width: {reset_width}, number of trials: {trials_num}')
    ...
    if overwrite_old_file and win_width == win_width_values[0]:  # Only overwrite for the first width
